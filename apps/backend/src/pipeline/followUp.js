...
function fallbackText(lead) {
  const interest = [lead.interested_country?.replace(/-/g, ' '), lead.interested_course?.toUpperCase()]
    .filter(Boolean).join(' ');
  return interest
    ? `Hello! Following up as promised from SkyHigh Educational Services — are you still considering ${interest}? Happy to pick up right where we left off. 😊`
    : 'Hello! Following up as promised from SkyHigh Educational Services — shall we continue where we left off? 😊';
}
